...
from lib.networks import EMCADNet  # uses your existing model
...
class CVCDataset(Dataset):
    ...
    def _load_pair(self, name: str) -> Tuple[Image.Image, Image.Image]:
        ...
        # masks may be palette/gray; convert to single channel
        msk = Image.open(msk_p).convert('L')
        return img, msk

    def _random_augment(self, img: Image.Image, msk: Image.Image):
        # geometric augments (keep in sync for both image and mask)
    ...
    def __getitem__(self, idx: int):
        ...
        # augment first (on original resolution) then resize
        if self.augment:
        ...
        # mask to {0,1} as float32 in shape [1,H,W]
        msk_np = np.array(msk, dtype=np.uint8)              # <-- convert PIL to numpy
...
def split_names(names: List[str], val_ratio: float, seed: int) -> Tuple[List[str], List[str]]:
    ...
    val = names[:n_val]
    train = names[names.index(val[-1]) + 1:] if len(names) > n_val else []
    # Simpler split keeping sizes correct
    train = names[n_val:]
    return train, val
...
class DiceLossMC(nn.Module):
    """
    Multi-class Dice loss on logits. Uses softmax and one-hot target.
    For binary segmentation, num_classes=2; averages dice over classes.
    """
    ...
    def forward(self, logits: torch.Tensor, target: torch.Tensor, softmax: bool = True) -> torch.Tensor:
        ...
        onehot = F.one_hot(target, num_classes=self.C).permute(0, 3, 1, 2).float()  # [N,C,H,W]
        ...
        if self.exclude_bg and self.C > 1:
            dice = dice_c[1:].mean()
        else:
            dice = dice_c.mean()
        return 1.0 - dice
...
def validate(model, loader, device):
    ...
    for imgs, gts in loader:
        imgs = imgs.to(device)
        # gts: [N,1,H,W] float -> long labels [N,H,W]
...
def build_loaders(args):
    ...
    train_ds = CVCDataset(img_dir, msk_dir, train_names, img_size=args.img_size, augment=True, normalize=True)
    val_ds   = CVCDataset(img_dir, msk_dir, val_names,   img_size=args.img_size, augment=False, normalize=True)
...
def main():
    ...
    ap.add_argument('--data_root', type=str, default='./data/CVC-ColonDB', help='folder with images/ and masks/')
    ...
    ap.add_argument('--out', type=str,        default='./model_pth/CVC-ColonDB')
    args = ap.parse_args()
    ...
    for epoch in range(args.epochs):
        ...
        for imgs, gts in pbar:
            imgs = imgs.to(device)
            gtl = (gts > 0.5).long().squeeze(1).to(device)  # [N,H,W] long
        ...
        if val_dice > best_dice:
            best_dice = val_dice
            torch.save(model.state_dict(), os.path.join(args.out, 'best_colondb.pth'))
        torch.save(model.state_dict(), os.path.join(args.out, 'last_colondb.pth'))
