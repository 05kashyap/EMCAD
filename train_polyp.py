...
from lib.networks import EMCADNet  # ...existing code...

IMG_EXTS = {'.png', '.jpg', '.jpeg', '.tif', '.tiff'}


class GenericPolypDataset(Dataset):
    def __init__(self, img_dir: str, msk_dir: str, names: List[str], img_size: int = 224, augment: bool = False, normalize: bool = True):
        super().__init__()
        self.img_dir, self.msk_dir = img_dir, msk_dir
        self.names, self.img_size = names, img_size
    ...
    def _load_pair(self, name: str):
        ...
        msk = Image.open(os.path.join(self.msk_dir, name)).convert('L')
        return img, msk
...
def discover_pairs(img_dir: str, msk_dir: str) -> List[str]:
    ...
    names = [f for f in imgs if os.path.exists(os.path.join(msk_dir, f))]
    names.sort()
    if not names:
...
def read_names(list_path: str) -> List[str]:
...
class DiceLossMC(nn.Module):
    def __init__(self, num_classes: int = 2, smooth: float = 1e-5, exclude_bg: bool = False):
        super().__init__()
        self.C, self.smooth, self.exclude_bg = num_classes, smooth, exclude_bg
...
def validate(model, loader, device):
    ...
    for imgs, gts in loader:
        ...
        tot_dice_fg += (2.0 * inter / denom).item()
        n += 1
    model.train()
    return (tot_loss / max(n, 1)), (tot_dice_fg / max(n, 1))


def resolve_dirs(dataset: str, root: str) -> Tuple[str, str]:
    ds = dataset.lower()
    if ds == 'kvasir':
        img_dir = os.path.join(root, "Kvasir-SEG", "Kvasir-SEG", "images")
        msk_dir = os.path.join(root, "Kvasir-SEG", "Kvasir-SEG", "masks")
    else:
...
def build_loaders(args):
    img_dir, msk_dir = resolve_dirs(args.dataset, args.data_root)
    if args.train_list and os.path.isfile(args.train_list) and args.val_list and os.path.isfile(args.val_list):
        train_names = read_names(args.train_list)
        val_names = read_names(args.val_list)
    else:
        names = discover_pairs(img_dir, msk_dir)
        train_names, val_names = split_names(names, args.val_ratio, args.seed)

    train_ds = GenericPolypDataset(img_dir, msk_dir, train_names, img_size=args.img_size, augment=True,  normalize=True)
    val_ds   = GenericPolypDataset(img_dir, msk_dir, val_names,   img_size=args.img_size, augment=False, normalize=True)

    if len(train_ds) == 0 or len(val_ds) == 0:
        raise ValueError(f'Empty dataset. images={img_dir}, masks={msk_dir}')
...
def main():
    ...
    ap.add_argument('--dataset', type=str, choices=['etis', 'cvc', 'kvasir'], default='kvasir')
    ...
    # Adjust defaults based on dataset if user didn't override
    if ap.get_default('data_root') == args.data_root:
        if args.dataset == 'etis':   args.data_root = './data/etis'
        if args.dataset == 'cvc':    args.data_root = './data/CVC-ColonDB'
    if ap.get_default('out') == args.out:
        if args.dataset == 'etis':   args.out = './model_pth/etis'
        if args.dataset == 'cvc':    args.out = './model_pth/CVC-ColonDB'
    ...
    for epoch in range(args.epochs):
        ...
        if val_dice > best_dice:
            best_dice = val_dice
            torch.save(model.state_dict(), os.path.join(args.out, f'best_{args.dataset}.pth'))
        torch.save(model.state_dict(), os.path.join(args.out, f'last_{args.dataset}.pth'))
